# Getting Started

New to LeanCorpus? Follow the first three pages in order to create an index, add documents and run your first search.

- [Installation and first index](01-installation.md)
- [Field types](02-fields.md)
- [Writer configuration](03-configuration.md)
- [Source-generated mapping](04-source-generated-mapping.md)
- [Configuration reference](05-configuration-reference.md)
