...
- Added a real `FstReader` over the FST1 blob format with arc-walk exact lookup, prefix enumeration, and automaton intersection; extended `LevenshteinAutomaton` with `MinDistance(state)` so fuzzy callers can recover edit distance from the FST traversal.
