...
namespace Rowles.LeanLucene.Tests.CompressionParity;
...
public sealed class CompressionCodecParityTests
{
    ...
    [Theory]
    [MemberData(nameof(PolicyCases))]
    public void Decompress_PayloadWithDroppedPrefix_Throws(FieldCompressionPolicy policy)
    {
        var payload = CreatePayload(4 * 1024);
        var codec = CompressionCodecRegistry.Get(policy);
        var compressed = codec.Compress(payload);
        var truncated = compressed.AsSpan(1).ToArray();

        Assert.ThrowsAny<Exception>(() => codec.Decompress(truncated, payload.Length));
    }
