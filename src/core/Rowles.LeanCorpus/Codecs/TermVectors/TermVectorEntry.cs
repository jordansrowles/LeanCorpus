...
/// <param name="StartOffsets">Optional start character offsets aligned to <paramref name="Positions"/>.</param>
/// <param name="EndOffsets">Optional end character offsets aligned to <paramref name="Positions"/>.</param>
public readonly record struct TermVectorEntry(string Term, int Freq, int[] Positions, byte[]?[]? Payloads = null,
    int[]? StartOffsets = null, int[]? EndOffsets = null);
